function [analyzed_sturct,pdata]=get_whisking_stats_new(wdata,hdata,headsize,plotting)
...
for i=1:length(wdata)
    ...
    if(numel(hhdata))
        Vx=diff(hhdata(:,2))*fs;
        Vy=diff(hhdata(:,3))*fs;
        V=sqrt(Vx.^2+Vy.^2); %head velocity
        V=[V(1);V]';
        V=V/headsize(i); %normalize to head size
        hs=headsize(i);
    else
        V=zeros(size(ddata))';
        hs=10;
    ...
    while(numel(ddata)>maxlag) % >1s
        ...
        [decomposed_data]=decompose_whisking_new(ddata,hhdata,fs,mode,hs);
        d(m)=duration;
        ...
        all_pump_hspeed=[all_pump_hspeed;V(decomposed_data.pump_ind)];
...
%         ind_pumps=decomposed_data.ind_pumps;
        ...
        SP1=reshape(SP',[],1);
        PH1=reshape(PH',[],1);
        PI=reshape(decomposed_data.pump_ind',[],1);
...
analyzed_sturct.all_pump_hspeed=all_pump_hspeed;
