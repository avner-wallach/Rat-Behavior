function [decomposed_struct]=decompose_whisking_new(wsig,hsig,fs,mode,headsize)
...
Vy=diff(hsig(:,3))*fs;
V=sqrt(Vx.^2+Vy.^2); 
V=[V(1);V]/headsize; % normalized head velocity
beta=hsig(:,1); %head direction
da=diff(beta)*fs; %head rotation: negative=leftward 
da=[da(1);da];
...
minamp=2.5; %minimal amplitude for active whisking
...
d_da=da(segmented_struct.ind_crossings); 
...
diluted_amp_off=[d_amp(ind_dilute)' d_offset(ind_dilute)' d_V(ind_dilute) d_da(ind_dilute)];
...
decomposed_struct.pump_ind=segmented_struct.pump_ind;
