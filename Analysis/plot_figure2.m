...
function [pvalue,N_pumps]=pump_bootstrap(strct,inds)
global minamp;
S=numel(strct);
if(nargin<2)
    for i=1:S
        inds{i}=1:numel(strct{i}.rise_fall(:,1));
    end
end
N=5e4;
for i=1:S
    ind=inds{i}(find(strct{i}.rise_fall(inds{i},1)>minamp));
    N_pumps(i,1)=numel(ind);
    pump_str{i,1}=strct{i}.all_pump_stregth(ind,1);    
    ind=inds{i}(find(strct{i}.rise_fall(inds{i},2)>minamp));
    N_pumps(i,2)=numel(ind);
    pump_str{i,2}=strct{i}.all_pump_stregth(ind,2);    
end

pvalue=nan(S-1,S-1,2);
for k=1:2
    for i=1:S-1
        for j=(i+1):S
            x=pump_str{i,k};
...
%                 VV=V(randperm(numel(V))); %no replacments
            ...
            pvalue(i,j-1,k)=sum(abs(dd)>=abs(D))/N;
%             if(D>0)
