...
N=50;
P=0.3;
[h_vals,H_vals,pvals,D,dd]=compare_entropy_head({hf_struct,ctl_struct},N,P,head);
pvals
plot_metrics(H_vals,h_vals,COL([2 1 2],:));
set(gca,'XTick',[],'Ylim',[5 8]);

figure;
for i=1:3
    H=bar(i,H_vals(i));
    set(H,'FaceColor',COL(mod(i,2)+1,:),'EdgeColor',COL(mod(i,2)+1,:));
    hold on;
end
set(gcf,'Position',[680 558 150 150]);
set(gca,'XTick',[],'Xlim',[0.5 3.5],'Ylim',[5 8]);
set(gca,'FontSize',16);
...
A=plotCDFComp(hf_struct.amp_off(hf_struct.amp_off(:,1)<=minamp,2),comp_struct.amp_off(comp_struct.amp_off(:,1)<=minamp,2),[COL(1,:);COL(2,:)]);
set(A(1),'XLim',[40 120]);
...
function [h_vals,H_vals,pvals,D,dd]=compare_entropy_head(strcts,N,P,head)
global m M midoff minamp maxamp fsize;
%maxoff=120;
ctrs{1}=linspace(minamp,maxamp,25);
ctrs{2}=linspace(m,M,25);
...
for i=1:(J-1)
    for j=(i+1):J
        [pvals(i,j-1),D_,dd_]=boottest(h_vals{i},h_vals{j},1e4,@mean,'double');
        if(i==1 & j==2)
            D(1)=D_;
            dd{1}=dd_;
        elseif(i==2 & j==3)
...
function [h_vals,H_vals,pvals]=compare_entropy_head_rand(strcts,N,P,head)
global m M midoff minamp maxamp fsize;
%maxoff=120;
ctrs{1}=linspace(minamp,maxamp,25);
ctrs{2}=linspace(m,M,25);
...
L_boot=min(L*P);
for i=1:J
    iind=randi(size(amp_off{i},1),L_boot,N);
    for n=1:N           
        a=amp_off{i}(iind(:,n),:);
        ind=find(a(:,1)>minamp & a(:,2)<M);
        Nx=hist3([a(ind,1) a(ind,2)],ctrs);
...
function plot_metrics(metric,mk,COL1)
global fsize
F=figure;
AA=axes;
X=[];
...
boxplot(X,G,'plotstyle','compact','symbol','','colorgroup',G,'median','line','positions',positions,...
    ...
    'orientation','vertical','LabelOrientation','inline','Whisker',0.7193,'Labels',{' ',' ',' '});
A=findobj('Tag','Box');
for i=1:numel(A)
...
function plot_metrics_violin(metric,mk,COL1)
global fsize
F=figure;
AA=axes;
violin(mk,'facecolor',COL1,'edgecolor',0.5*[1 1 1],'mc',[],'medc',[]);
legend('off');
% axis([0.5 numel(metric)+.5 min(m)*.9 max(M)*1.1])
box('off');
set(AA,'FontSize',fsize);
